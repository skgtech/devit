    ...
    rules: [
      {
        ...
        use: [
          {
            loader:
              "url-loader?limit=10000&mimetype=application/font-woff&name=assets/js/[hash].[ext]"
          }
        ]
      },
      {
        test: /\.(ttf|eot|svg)(\?v=[0-9]\.[0-9]\.[0-9])?(\?\-[0-9|a-z]{6})?$/,
        use: [{ loader: "file-loader?name=assets/js/[hash].[ext]" }]
      },
      {
        test: /\.(sass|scss)?$/,
        use: [
      ...
      {
        test: /\.(css)?$/,
        use: [{ loader: "style-loader" }, { loader: "css-loader?minimize" }]
      }
    ]
  },
  resolve: {
